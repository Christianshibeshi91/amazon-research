...
import { storeIntelligenceReport } from "@/lib/stores/reportStore";
...
export async function POST(request: NextRequest) {
  ...
  // Extract user ID from auth header — required for this endpoint
  ...
  try {
    ...
    // Auth verification failed — reject the request
    return new Response(
  ...
  try {
    ...
    if (typeof body.availableCapital === "number" && !Number.isNaN(body.availableCapital)) {
  ...
  // Validate — explicitly reject NaN
  if (Number.isNaN(availableCapital) || availableCapital < 2000 || availableCapital > 5000) {
    return new Response(
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // Atomically increment user's report counter using FieldValue.increment
        if (userId) {
          try {
            const db = getAdminDb();
            const userRef = db.collection("users").doc(userId);
            await userRef.update({
              intelligenceReportsRun: FieldValue.increment(1),
            });
          } catch {
